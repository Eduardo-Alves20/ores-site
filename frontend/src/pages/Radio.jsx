...
export default function WebRadio() {
  ...
  return (
    <div className="animate-page">
      <PageHeader eyebrow="Comunicação" title="Web Rádio PES" subtitle="Ouça nossa programação com louvor, homilias e reflexões 24h." />
      <section style={{ padding:'72px 24px', maxWidth:800, margin:'0 auto' }}>
        <Reveal>
          <div style={{ background:'var(--navy)', borderRadius:20, padding:'56px', textAlign:'center' }}>
            ...
            <h2 style={{ fontFamily:'Playfair Display,serif', fontSize:28, color:'#fff', fontWeight:700, marginBottom:8 }}>Rádio Espírito Santo</h2>
            <p style={{ fontSize:15, color:'rgba(255,255,255,.55)', marginBottom:40 }}>Ao vivo · São José dos Campos/SP</p>

            {/* Fake audio bars animation */}
            ...
            {streamUrl ? (
              <button onClick={() => setPlaying(!playing)} style={{ width:72, height:72, borderRadius:'50%', background:'var(--gold)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 24px', boxShadow:'0 8px 32px rgba(184,148,90,.5)', transition:'transform .2s', border:'none' }}
              ...
              <div style={{ padding:'16px 32px', borderRadius:12, background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)' }}>
                <p style={{ color:'rgba(255,255,255,.45)', fontSize:14 }}>Stream ainda não configurado. Configure a URL no painel administrativo.</p>
              </div>
            )}
