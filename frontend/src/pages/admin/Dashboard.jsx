...
function StatCard({ icon, label, value, color, to }) {
  return (
    <Link to={to} style={{ background:'#fff', borderRadius:12, border:'1px solid var(--border)', padding:'24px', display:'flex', alignItems:'center', gap:16, textDecoration:'none', transition:'box-shadow .2s,transform .2s' }}
  ...
  return (
    <div>
      <div style={{ marginBottom:32 }}>
        <h1 style={{ fontFamily:'Playfair Display,serif', fontSize:28, fontWeight:700, color:'var(--navy)', marginBottom:4 }}>Olá, {user?.name?.split(' ')[0]}!</h1>
        <p style={{ fontSize:14, color:'var(--text-soft)' }}>Bem-vindo ao painel administrativo da Paróquia Espírito Santo.</p>
      </div>
      ...
      {loading ? <p style={{ color:'var(--text-soft)' }}>Carregando...</p> : (
        <>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))', gap:16, marginBottom:32 }}>
            ...
            <StatCard icon={<Newspaper size={22}/>} label="Notícias publicadas" value={stats.news ?? '-'} color="#6366f1" to="/admin/news" />
            <StatCard icon={<MessageSquare size={22}/>} label="Mensagens não lidas" value={stats.unreadMessages ?? '-'} color="#ef4444" to="/admin/messages" />
